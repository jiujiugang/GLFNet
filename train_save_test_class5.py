...
def whole_face_block_coordinates():
    df = pandas.read_csv(r'D:\ZP\5casme.csv')
    m, n = df.shape    #获取CSV文件中的样本数量 m 和列数 n
    base_data_src = r'D:\ZP\datasets_5_class\apex'  #设置图像的基本目录路径
    total_emotion = 0
    image_size_u_v = 28
    # get the block center coordinates
    face_block_coordinates = {}  #用于存储每张图像的关键点坐标（即人脸块的中心坐标）的字典
    ...
    for i in range(0, m):
        ...
        face_apex = cv2.resize(train_face_image_apex, (image_size_u_v, image_size_u_v), interpolation=cv2.INTER_AREA)
        ...
        if batch_landmarks is None:
            print(f"Warning: No landmarks detected for image {image_name}. Using default landmarks.")
            batch_landmarks = np.array([[[9.528073, 11.062551], [21.396168, 10.919773],
                                         [15.380184, 17.380562], [10.255435, 22.121233],
                                         [20.583706, 22.25584]]])

        row_n, col_n = np.shape(batch_landmarks[0])
        for j in range(0, row_n):  # 注意使用不同的循环变量
            for k in range(0, col_n):
                batch_landmarks[0][j][k] = np.clip(batch_landmarks[0][j][k], 7, 21)  # 限制坐标范围

        batch_landmarks = batch_landmarks.astype(int)
        face_block_coordinates[image_name] = batch_landmarks[0]

    print(f"Processed {len(face_block_coordinates)} images with landmarks.")
    return face_block_coordinates
"""

def whole_face_block_coordinates():
    df = pd.read_csv(r'D:\HTNet-master\HTNet-master\CasmeII_5class.csv')
    m, n = df.shape
    base_data_src = r'D:\HTNet-master\HTNet-master\datasets_5_class\CASMEII_APEX'
    image_size_u_v = 28
    face_block_coordinates = {}
    no_landmark_count = 0  # 用于统计未被标记的图像数量
    ...
    # 打印未被标记的图像数量
    print(f"Total number of images with no landmarks detected: {no_landmark_count}")

    return face_block_coordinates
"""

def whole_face_block_coordinates():
    df = pandas.read_csv(r'D:\ZP\five_samm.csv')
    m, n = df.shape    # 获取CSV文件中的样本数量 m 和列数 n
    base_data_src = r'D:\ZP\datasets_5_class\samm_apex'  # 设置图像的基本目录路径
    total_emotion = 0
    image_size_u_v = 28
    # get the block center coordinates
    face_block_coordinates = {}  # 用于存储每张图像的关键点坐标（即人脸块的中心坐标）的字典

    total_detected = 0  # 统计检测到 landmarks 的图像数
    total_not_detected = 0  # 统计未检测到 landmarks 的图像数
    ...
    for i in range(0, m):
        ...
        if batch_landmarks is None:
            ...
            batch_landmarks = np.array([[[9.528073, 11.062551], [21.396168, 10.919773],
                                         [15.380184, 17.380562], [10.255435, 22.121233],
                                         [20.583706, 22.25584]]])
            total_not_detected += 1  # 增加未检测到的数量
        else:
            total_detected += 1  # 增加检测到的数量
    ...
    print(f"Processed {len(face_block_coordinates)} images with landmarks.")
    print(f"Detected landmarks in {total_detected} images.")
    print(f"Did not detect landmarks in {total_not_detected} images.")
    return face_block_coordinates
"""
def crop_optical_flow_block():
    face_block_coordinates_dict = whole_face_block_coordinates()
    whole_optical_flow_path = r'D:\HTNet-master\HTNet-master\datasets_5_class\CASMEII_5_CLASS_ALL'
...
def main(config):
    ...
    if config.train and not os.path.exists('ourmodel_threedatasets_casme550'):
        os.mkdir('ourmodel_threedatasets_casme550')
    ...
    main_path = r'D:\HTNet-master\HTNet-master\datasets_5_class\CASMEII_5class_flow'
    ...
    for n_subName in subNames:
        ...
        if config.train:
            ...
            for epoch in range(epochs):
                ...
                # 在第一个 epoch 保存初始模型
                if epoch == 0:  # 保存初始模型
                    weight_path = f'ourmodel_threedatasets_casme550/best_model_{n_subName}.pth'
                    torch.save(model.state_dict(), weight_path)
                    print(f'Saved initial model at {weight_path}')
                ...
                UF1, UAR = recognition_evaluation(all_labels, all_predicted)
                print(f'Test Accuracy of {n_subName} at Epoch {epoch + 1}: {subject_accuracy:.2f}%')
                print(f'Test UF1: {UF1:.4f}, Test UAR: {UAR:.4f}')
                """
                if subject_accuracy == 100.0:  # 如果测试准确率为100%
                    directory = 'ourmodel_threedatasets_weight22'
                    if not os.path.exists(directory):
                        os.makedirs(directory)  # 创建目录
                    weight_path_subject = f'{directory}/{n_subName}_best_accuracy.pth'
                    torch.save(model.state_dict(), weight_path_subject)
                    print(f"Saved model with 100% accuracy for subject: {n_subName}")
                    break  # 提前停止训练
                    """

                if subject_accuracy > best_accuracy_for_each_subject:
                    best_accuracy_for_each_subject = subject_accuracy
                    weight_path = f'ourmodel_threedatasets_casme550/best_model_{n_subName}.pth'
                    torch.save(model.state_dict(), weight_path)
        ...
        if config.test:
            ...
            if os.path.exists(weight_path):
                ...
                print(f'No best model found for {n_subName}, using current model')
            #model.load_state_dict(torch.load(
                #f'ourmodel_threedatasets_casme5最优/best_model_{n_subName}.pth'))
            model.eval()
            subject_correct, subject_total = 0, 0
            all_labels, all_predicted = [], []
            ...
            # 输出当前主体的预测和真实标签
            print(f'Best Predicted for {n_subName}: {all_predicted}')
            print(f'Ground Truth for {n_subName}: {all_labels}')
        ...
        # 输出所有主体的预测和真实标签
        print('Overall Prediction and Ground Truth:')
