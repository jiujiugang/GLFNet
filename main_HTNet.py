...
def whole_face_block_coordinates():#根据图像中的人脸特征提取出人脸块的中心坐标。
    ...
    for i in range(0, m):#一个for循环，它将从0开始迭代到m-1
        ...
        # print(image_name)
        img_path_apex = base_data_src + '/' + df['imagename'][i]#这行代码构建了一个图像文件的完整路径。它将基础数据源路径base_data_src与数据框中'imagename'列的值拼接起来
        train_face_image_apex = cv2.imread(img_path_apex) # (444, 533, 3)
        face_apex = cv2.resize(train_face_image_apex, (28,28), interpolation=cv2.INTER_AREA)#这行代码将读取的图像大小调整为28x28像素，使用INTER_AREA插值方法。
        ...
        if batch_landmarks is None:
            #print(f"Warning: No landmarks detected for image {image_name}")
        ...
        # print(batch_landmarks[0])
        # get the block center coordinates
        face_block_coordinates[image_name] = batch_landmarks[0]#将检测到的关键点坐标（batch_landmarks[0]）存储在一个名为face_block_coordinates的字典中
...
class Fusionmodel(nn.Module):
   def __init__(self):
     ...
     self.fc1 = nn.Linear(15, 3) # 15->3#这行代码创建了一个全连接层（线性层），输入特征维度是15，输出特征维度是3。
     self.bn1 = nn.BatchNorm1d(3)#创建了一个批量归一化层，用于对输入数据进行归一化处理
     self.d1 = nn.Dropout(p=0.5)
     # Linear 256 to 26
     self.fc_2 = nn.Linear(6, 2) # 6->3#全连接层，该层的输入特征维度是6，输出特征维度是2
   ...
   def forward(self, whole_feature, l_eye_feature, l_lips_feature, nose_feature, r_eye_feature, r_lips_feature):
     ...
     #这行代码使用torch.cat函数将五个特征（左眼、左唇、鼻子、右眼和右唇特征）沿着第一个维度（即维度0，通常代表批量大小）进行拼接。这得到了一个融合了五个特征的张量
     # nn.linear - fc
     fuse_out = self.fc1(fuse_five_features)#这行代码将拼接后的特征fuse_five_features传递给全连接层self.fc1，并得到输出fuse_out
     # fuse_out = self.bn1(fuse_out)
     fuse_out = self.relu(fuse_out)#这行代码对fuse_out应用ReLU激活函数。
     fuse_out = self.d1(fuse_out) # drop out

     fuse_whole_five_parts = torch.cat(
         (whole_feature,fuse_out), 0)#这行代码将整体特征whole_feature和经过处理的特征fuse_out沿着第一个维度进行拼接，得到一个新的张量fuse_whole_five_parts
     # fuse_whole_five_parts = self.bn1(fuse_whole_five_parts)
     fuse_whole_five_parts = self.relu(fuse_whole_five_parts)#线性激活
     fuse_whole_five_parts = self.d1(fuse_whole_five_parts)  # drop out
     out = self.fc_2(fuse_whole_five_parts)#将fuse_whole_five_parts传递给全连接层self.fc_2，并得到最终的输出out。
     return out
    ...
    for n_subName in subName:#遍历主数据集下的每个子文件夹
        ...
        for n_expression in expression:#遍历训练集文件夹下的每个表情类别
            ...
            for n_img in img:
                ...
                four_parts_train.append(lr_eye_lips)#将拼接后的光学流动数据添加到four_parts_train列表中。
        ...
        print('Best Predicted    :', best_each_subject_pred)#初始化和字典设置：
        accuracydict = {}
        accuracydict['pred'] = best_each_subject_pred
        accuracydict['truth'] = y.tolist()#accuracydict 是一个字典，用来保存每个子任务（或每个样本）的预测结果 (pred) 和真实标签 (truth)。
