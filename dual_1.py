...
def main(config):
    ...
    if config.train:
        if not path.exists('ourmodel_threedatasets_weight'):
            os.mkdir('ourmodel_threedatasets_weight')
    ...
    for n_subName in subName:
        ...
        weight_path = 'ourmodel_threedatasets_weight' + '/' + n_subName + '.pth'
        ...
        for epoch in range(1, epochs + 1):
            ...
            if best_accuracy_for_each_subject <= val_acc:  # 它检查当前的验证准确率是否大于等于之前记录的最佳准确率。
                ...
                temp_best_each_subject_pred.extend(torch.max(yhat, 1)[
                                                       1].tolist())  # 这一行代码将当前批次的预测结果（yhat）中每个样本的最大概率对应的类别索引提取出来，并加入到 temp_best_each_subject_pred 列表中。
                best_each_subject_pred = temp_best_each_subject_pred  # 更新记录每个子主题的最佳预测结果
                # Save Weights
                if val_acc == 1.0:  # 如果测试准确率为100%
                    directory = 'ourmodel_threedatasets_weight'
                    if not os.path.exists(directory):
                        os.makedirs(directory)  # 创建目录
                    weight_path_subject = f'ourmodel_threedatasets_weight/{n_subName}_best_accuracy.pth'
