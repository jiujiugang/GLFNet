...
from model_start_4 import FusionModel
...
def whole_face_block_coordinates():
    ...
    m, n = df.shape
    base_data_src = r'D:\HTNet-master\HTNet-master\datasets_5_class\samm_apex'
    image_size_u_v = 28
...
def crop_optical_flow_block():
    ...
    whole_optical_flow_path = r'D:\HTNet-master\HTNet-master\datasets_5_class\newsamm55'
...
def main(config):
    ...
    main_path = r'D:\HTNet-master\HTNet-master\datasets_5_class\new-samm5'
    ...
    for n_subName in subNames:
        ...
        if config.test:
            ...
            model.eval()
            subject_correct, subject_total = 0, 0  # 修改变量名称
            all_labels, all_predicted = [], []
            ...
            subject_accuracy = 100. * subject_correct / subject_total  # 计算每个主题的准确率
            UF1, UAR = recognition_evaluation(all_labels, all_predicted)
            print(f'Test Accuracy of {n_subName}: {subject_accuracy:.2f}%')
            print(f'Test UF1: {UF1:.4f}, Test UAR: {UAR:.4f}')
            all_accuracy_dict[n_subName] = (subject_accuracy, UF1, UAR)

            overall_correct += subject_correct  # 累积每个主题的正确预测数
            overall_total += subject_total  # 累积每个主题的总样本数
            total_gt.extend(all_labels)
            total_pred.extend(all_predicted)

    total_accuracy = 100. * overall_correct / overall_total  # 计算总准确率
