...
class FusionModel(nn.Module):#MUL
    ...
    def forward(self, global_x, parts_x):
        ...
        if x.size(2) == 1 or x.size(3) == 1:
            x = self.final_upsample(x)#如果融合后的特征图 x 在空间维度 (height 或 width) 上的尺寸为1，则通过 self.final_upsample 进行上采样，将 x 的空间尺寸调整为 (28, 28)。这可以避免因为尺寸太小导致的信息丢失。

        x = torch.flatten(x, start_dim=1)#将特征图 x 展平（flatten），将空间维度 (height, width) 展平为一个长向量，使其可以传递到全连接层中。start_dim=1 表示从第一个维度开始展平，保留批次维度。
