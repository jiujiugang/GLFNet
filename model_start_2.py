...
class StarNet(nn.Module):
    def __init__(self, base_dim=256, depths=[3,4], mlp_ratio=4, drop_path_rate=0.0, **kwargs):
        super().__init__()
        ...
        self.final_conv = nn.Conv2d(self.in_channel, 1024, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x = self.final_conv(x)  # Ensure the output channel size is 1024
        return x
...
def starnet_s1(pretrained=True, **kwargs):
    model = StarNet(base_dim=128, depths=[3, 3, 3, 3], **kwargs)
    if pretrained:
...
class FusionModel(nn.Module):
    def __init__(self, num_classes=5):
        ...
        self.StarNet = StarNet(base_dim=128, depths=[3,4], mlp_ratio=4, drop_path_rate=0.0)
        self.HTnet = HTNet(
            image_size=224,
            patch_size=7,
            dim=256,
        ...
        self.final_conv = nn.Conv2d(256, 1024, kernel_size=1)  # Ensure HTnet output channels are adjusted
    ...
    def forward(self, global_x, parts_x):
        ...
        # Ensure parts_features has 1024 channels
        if parts_features.size(1) != 1024:
        ...
        x = global_features * parts_features
...
"""startnet5class4block
class FusionModel(nn.Module):
    def __init__(self, num_classes=CASMEII_5CLASS_train):
        super(FusionModel, self).__init__()
        ...
        self.dropout1 = nn.Dropout(0.CASMEII_5CLASS_train)
        self.dropout2 = nn.Dropout(0.CASMEII_5CLASS_train)
        self.dropout3 = nn.Dropout(0.CASMEII_5CLASS_train)
...
"""
class FusionModel(nn.Module):#3class
    def __init__(self, num_classes):
        ...
        self.fc1 = nn.Linear(1024 * 8 * 8, 256)  # Adjust input size to 65536
        self.fc2 = nn.Linear(256, 128)
        ...
        self.final_conv = nn.Conv2d(1024, 1024, kernel_size=1)  # Ensure channels match after multiplication
        self.final_upsample = nn.Upsample(size=(28, 28), mode='bilinear', align_corners=False)
    ...
    def forward(self, global_x, parts_x):
        ...
        # Adjust the size of parts_features to match global_features if necessary
        if global_features.size() != parts_features.size():
            parts_features = F.interpolate(parts_features, size=global_features.shape[2:], mode='bilinear', align_corners=False)

        # Ensure global_features and parts_features have the same number of channels before element-wise multiplication
        if global_features.size(1) != parts_features.size(1):
            parts_features = self.final_conv(parts_features)

        # Element-wise multiplication
        x = global_features * parts_features
        # # Ensure the feature map size does not get too small
        if x.size(2) == 1 or x.size(3) == 1:
            x = self.final_upsample(x)
...
class FusionModel(nn.Module):
    def __init__(self, num_classes):
        ...
        self.fc1 = nn.Linear(131072, 256)  # Adjust input size if necessary
        self.fc2 = nn.Linear(256, 128)
        self.fc3 = nn.Linear(128, num_classes)
    ...
    def forward(self, global_x, parts_x):
        ...
        # Ensure global_features and parts_features have the same number of channels before concatenation
        if global_features.size(1) != parts_features.size(1):
            parts_features = self.final_conv(parts_features)

        x = torch.cat((global_features, parts_features), dim=1)
        x = torch.flatten(x, start_dim=1)
        x = F.relu(self.fc1(x))
        x = self.dropout1(x)
        x = F.relu(self.fc2(x))
...
class FusionModel(nn.Module):
    def __init__(self, num_classes):
        ...
        self.fc3 = nn.Linear(128, num_classes)
        # Dropout layers
        self.dropout1 = nn.Dropout(0.CASMEII_5CLASS_train)  # 添加 dropout，丢弃概率为 0.CASMEII_5CLASS_train
        self.dropout2 = nn.Dropout(0.CASMEII_5CLASS_train)
        self.dropout3 = nn.Dropout(0.CASMEII_5CLASS_train)
        # Add a final convolutional layer to adjust the output to (batch_size, 3, 28, 28)
        self.final_conv = nn.Conv2d(128, 3, kernel_size=1)
        self.final_upsample = nn.Upsample(size=(28, 28), mode='bilinear', align_corners=False)
    ...
    def forward(self, x):
        x1 = self.StarNet(x)
        x2 = self.HTnet(x)
        # Adjust the size of x2 to match x1 if necessary
        if x1.size() != x2.size():
...
if __name__ == "__main__":
    ...
    model = FusionModel(image_size=224, patch_size=7, dim=256, heads=3, num_hierarchies=3, block_repeats=(2, 2, 10),
                        num_classes=3)
