...
class StarNet(nn.Module):
    def __init__(self, base_dim=256, depths=[3, 4, 6, 3], mlp_ratio=4, drop_path_rate=0.0, **kwargs):
        super().__init__()
...
import torch.nn.functional as F
"""
class FusionModel(nn.Module):  # 拉格朗日插值
    def __init__(self, num_classes=3):
        super(FusionModel, self).__init__()
        self.StarNet = StarNet(base_dim=128, depths=[3, 4, 4, 3], mlp_ratio=4, drop_path_rate=0.0)
        self.HTnet = HTNet(
        ...
        self.fc1 = nn.Linear(1024 * 8 * 8, 256)
        self.fc2 = nn.Linear(256, 128)
        self.fc3 = nn.Linear(128, num_classes)  # 输出大小改为 num_classes
    ...
    def forward(self, global_x, parts_x):
        ...
        if global_features.size() != parts_features.size():
            # 获取 global_features 的目标尺寸
            target_height, target_width = global_features.shape[2:]

            # 根据需要的 y_coords 和 x_coords 进行插值
            y_coords = torch.arange(parts_features.size(2), device=parts_features.device)
            x_coords = torch.arange(parts_features.size(3), device=parts_features.device)

            # 用拉格朗日插值替代双线性插值
            parts_features = lagrange_interpolation(parts_features, y_coords, x_coords, target_height, target_width)

        if global_features.size(1) != parts_features.size(1):
            parts_features = self.final_conv(parts_features)

        x = global_features * parts_features  # 执行逐元素乘法，将 global_features 和调整后的 parts_features 相乘。

        if x.size(2) == 1 or x.size(3) == 1:
            x = self.final_upsample(x)  # 进行上采样操作。

        x = torch.flatten(x, start_dim=1)  # 将特征图展平
        x = F.relu(self.fc1(x))  # 传递到全连接层
        x = self.dropout1(x)
        x = F.relu(self.fc2(x))
        x = self.dropout2(x)
...
# 修改 least_squares_interpolation 函数，接受 size 参数
def least_squares_interpolation(input_tensor, target_size):
    # 使用 bilinear 插值将 input_tensor 调整为 target_size
...
def hermite_interpolation(input_tensor, target_size):
    """
    对 input_tensor 进行Hermite插值，调整到 target_size。
    这里使用梯度信息来近似Hermite插值。
    """
    # 首先对输入进行双线性插值，作为基准
    interp_tensor = F.interpolate(input_tensor, size=target_size, mode='bilinear', align_corners=False)

    # 计算梯度，模拟Hermite插值的导数信息
    dx = F.conv2d(interp_tensor, weight=torch.tensor([[[-1, 1]]], dtype=torch.float32).unsqueeze(0), padding=1)
    dy = F.conv2d(interp_tensor, weight=torch.tensor([[[[-1], [1]]]], dtype=torch.float32).unsqueeze(0), padding=1)

    # 根据梯度调整插值结果，模仿Hermite插值
    # 这里只是一个非常简化的实现，实际的Hermite插值会更复杂
    grad_x = torch.abs(dx)
    grad_y = torch.abs(dy)

    # 将梯度加权到插值结果中
    hermite_result = interp_tensor + 0.5 * (grad_x + grad_y)

    return hermite_result

"""
class FusionModel(nn.Module):  # MUL 最小二乘法
    def __init__(self, num_classes=3):
        super(FusionModel, self).__init__()
        self.StarNet = StarNet(base_dim=128, depths=[3, 4, 4, 3], mlp_ratio=4, drop_path_rate=0.0)
        self.HTnet = HTNet(
    ...
    def forward(self, global_x, parts_x):
        ...
        # Resize parts_features using least-squares interpolation
        if global_features.size() != parts_features.size():
            # 确保调用时使用 size 作为参数传递给 least_squares_interpolation
            parts_features = least_squares_interpolation(parts_features, target_size=global_features.shape[2:])#最小二乘法

        if global_features.size(1) != parts_features.size(1):
            parts_features = self.final_conv(parts_features)

        # Perform element-wise multiplication to fuse features
        x = global_features * parts_features  # Perform element-wise multiplication

        # If the feature map size is 1 in either spatial dimension, upsample
        if x.size(2) == 1 or x.size(3) == 1:
...
class FusionModel(nn.Module):  # MUL 卷积下采样
    def __init__(self, num_classes=3):
        super(FusionModel, self).__init__()
        self.StarNet = StarNet(base_dim=128, depths=[3, 4, 4, 3], mlp_ratio=4, drop_path_rate=0.0)
        self.HTnet = HTNet(
        ...
        # Add a 1x1 convolution to adjust the spatial dimensions of parts_features
        self.conv_to_match_size = nn.Conv2d(in_channels=1024, out_channels=1024, kernel_size=1)

    def forward(self, global_x, parts_x):
        global_features = self.StarNet(global_x)
        parts_features = self.HTnet(parts_x)

        # If the spatial dimensions of global_features and parts_features do not match,
        # use a convolution to adjust the size of parts_features
        if global_features.size(2) != parts_features.size(2) or global_features.size(3) != parts_features.size(3):
            parts_features = self.conv_to_match_size(parts_features)
